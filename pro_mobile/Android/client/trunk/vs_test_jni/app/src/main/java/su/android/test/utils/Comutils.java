...
//                    + "\nrequiredAccountType:" + packageInfo.requiredAccountType
        ...
        Logger.i(TAG, String.format("Screen,width=%1$s,height=%2$s,density=%3$s,densityDpi=%4$s"
                , width, height, density, densityDpi));
        final String os = "android";
        final String osver = android.os.Build.VERSION.RELEASE;
        final String brand = android.os.Build.BRAND;
        final String model = android.os.Build.MODEL;
        int localV = packageInfo.versionCode;
        String localVName = packageInfo.versionName;
        String version = String.format("版本号:%1$s,版本名称:%2$s", localV, localVName);
        String mobileType = String.format("OS:%1$s#%2$s,%3$s#%4$s", os, osver, brand, model);
        Logger.i(TAG, "version::"+version );
        Logger.i(TAG, "mobileType::"+mobileType);
