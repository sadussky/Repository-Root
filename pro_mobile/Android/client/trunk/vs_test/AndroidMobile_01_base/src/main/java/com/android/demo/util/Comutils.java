...
//                    + "\nrequiredAccountType:" + packageInfo.requiredAccountType
    ...
    public void useDexClassLoader() {
        Intent intent = new Intent("cn.jpush.android.ui.PushActivity", null);
        PackageManager pm = mContext.getPackageManager();
        final List<ResolveInfo> plugins = pm.queryIntentActivities(intent,PackageManager.MATCH_DEFAULT_ONLY);

        for (ResolveInfo tmp : plugins) {
            ResolveInfo rinfo = tmp;
            ActivityInfo ainfo = rinfo.activityInfo;
            String div = System.getProperty("path.separator");
            String packageName = ainfo.packageName;
            if (packageName.equals("com.zanfitness.coach")) {
                String dexPath = ainfo.applicationInfo.sourceDir;
                String dexOutPathDir = mContext.getApplicationInfo().dataDir;
                String libPath = ainfo.applicationInfo.nativeLibraryDir;
                Logger.i(TAG, "\npackageName:" + packageName
                        + "\ndexPath:" + dexPath
                        + "\ndexOutPathDir:" + dexOutPathDir
                        + "\nlibPath:" + libPath);
                DexClassLoader dexClassLoader = new DexClassLoader(dexPath, dexOutPathDir, libPath, this.getClass().getClassLoader());
                try {
                    ...
                    Method method = clazz.getMethod("function1", params);
                    Integer ret = (Integer) method.invoke(obj, 1, 1);
                    Logger.i(TAG, "\nreturn value is :" + ret);
